...
"""))
    ...
    if mode == MODE_GITHUB:
        cells.append(markdown(
            ...
            "Clones the repository into the Colab session and changes into it, "
            "so every relative path (`src/`, `results/`, `tests/`) resolves. "
            "Re-running the cell pulls the latest commit rather than cloning "
            "again, so you can pick up changes without restarting the "
            "runtime.\n\n"
            "**The repository is private**, and a Colab session holds no GitHub "
            "credentials, so an anonymous clone will fail. The cell then looks "
            "for a token in Colab Secrets (the key icon in the left sidebar) "
            "named `GITHUB_TOKEN`; if there isn't one it stops with "
            "instructions rather than failing obscurely three cells later. "
            "Making the repository public removes the need for a token "
            "entirely.\n\n"
            "The token is read from the secret store straight into the clone "
            "URL - it is never printed, and never written to disk."
        ))
        cells.append(code("""
...
def _clone(url):
    \"\"\"Clone, never echoing the URL - it may carry a token.\"\"\"
    return subprocess.run(["git", "clone", "--depth", "1", "--quiet",
                           url, CLONE_DIR],
...
if os.path.isdir(os.path.join(CLONE_DIR, ".git")):
    ...
    print("cloning https://" + REPO_HOST)
    result = _clone("https://" + REPO_HOST)

    if result.returncode != 0:
        # The repository is private, so Colab -- which holds no credentials --
        # cannot clone it anonymously. Look for a GitHub personal access token
        # stored in Colab Secrets (the key icon in the left sidebar) under the
        # name GITHUB_TOKEN. The token is read straight from the secret store
        # into the clone URL and is never printed or written to disk.
        token = None
        try:
        ...
        if token:
            print("anonymous clone failed (private repo) - retrying with "
                  "GITHUB_TOKEN from Colab Secrets")
            result = _clone("https://" + token + "@" + REPO_HOST)

        if result.returncode != 0:
            raise SystemExit(
                "Could not clone the repository.\\n\\n"
                "It is private, so Colab needs credentials. Either:\\n"
                "  1. Make the repo public on GitHub, then re-run this cell; or\\n"
                "  2. Create a fine-grained personal access token with read\\n"
                "     access to this repo, add it to Colab Secrets (key icon,\\n"
                "     left sidebar) named GITHUB_TOKEN, enable notebook access,\\n"
                "     and re-run; or\\n"
                "  3. Use REVIEW2_AR_EAURP.ipynb instead - it embeds every\\n"
                "     source file and needs no network at all."
            )
...
""".format(host=REPO_URL.replace("https://", ""), clone=CLONE_DIR)))
